...
SAVED_TENSORSETS_DIR  = "./MNIST/saved_tensorsets"
RESULTS_DIR           = "./MNIST/results_augmentation_search"
...
def load_tensor_datasets(save_dir=SAVED_TENSORSETS_DIR):
    ...
    train_tds = torch.load(os.path.join(save_dir, "MNIST_train_tensorset.pth"))
    val_tds   = torch.load(os.path.join(save_dir, "MNIST_val_tensorset.pth"))
    test_tds  = torch.load(os.path.join(save_dir, "MNIST_test_tensorset.pth"))
...
if __name__ == "__main__":
    ...
    pbounds = {
    "rotate_prob": (0.0, 0.7),      # Lower max prob than KMNIST (still optional)
    "rotate_deg": (0, 10),          # Reduced from 15° to 10° (avoid 6/9 confusion)
    "translate_prob": (0.0, 0.8),   # Keep moderate translation probability
    "translate_frac": (0, 0.2),     # Reduced from 0.3 (MNIST is tightly centered)
    "flip_prob": (0.0, 0.0),        # Keep disabled (digits are asymmetric)
    "noise_prob": (0.0, 0.5),       # Lower max prob (MNIST is cleaner)
    "noise_std": (0, 0.05),         # Smaller noise (avoid overwhelming thin strokes)
    "perspective_prob": (0.0, 0.5), # Lower prob than KMNIST
    "perspective_distortion": (0, 0.2), # Less distortion (preserve digit structure)
    "num_augmented_copies": (5, 5), # Fewer copies needed (MNIST has simpler data)
    "mixup_prob": (0.0, 0.0),       # Keep disabled (not critical for MNIST)
    "mixup_alpha": (0.0, 0.0),      # Disabled
    "cutout_prob": (0.0, 0.0),      # Keep disabled (can erase key strokes)
    "cutout_size": (0.0, 0.0)       # Disabled
}
    ...
    final_save_path = os.path.join(RESULTS_DIR, "MNIST_train_augmented_tensorset.pth")
