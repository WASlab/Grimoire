...
SAVED_TENSORSETS_DIR  = "./FashionMNIST/saved_tensorsets"
RESULTS_DIR           = "./FashionMNIST/results_augmentation_search"
...
def load_tensor_datasets(save_dir=SAVED_TENSORSETS_DIR):
    ...
    train_tds = torch.load(os.path.join(save_dir, "FashionMNIST_train_tensorset.pth"))
    val_tds   = torch.load(os.path.join(save_dir, "FashionMNIST_val_tensorset.pth"))
    test_tds  = torch.load(os.path.join(save_dir, "FashionMNIST_test_tensorset.pth"))
...
# ---------------------------
# Augmentation Pipeline Builder (GPU-accelerated for grayscale)
#   We remove hue/saturation from ColorJitter since they're meaningless in single-channel data,
#   but brightness/contrast can still help.
# ---------------------------
def build_augmentation_pipeline(policy):
    ...
    This version is intended for grayscale images. The main difference from the RGB version
    is that ColorJitter only uses brightness and contrast (no saturation/hue).
    """
    ops = []
    ...
    # ColorJitter (brightness/contrast only for grayscale)
    if policy.get("color_jitter_strength", 0) > 0:
        cjs = policy["color_jitter_strength"]
        # for grayscale: brightness=..., contrast=...
    ...
    if policy.get("scale_prob", 0) > 0:
        scale_transform = T.RandomResizedCrop(
            (28, 28),  # keep final size at 28x28
            scale=(0.8, 1.2),
...
def create_augmented_dataset_batched(original_dataset, augmentation_pipeline, num_copies, policy, batch_size_augment=2056):
    ...
    Expects grayscale images shaped (N, 1, H, W).
    """
    all_images = []
    all_labels = []
    ...
    images = torch.cat(all_images, dim=0)  # (N, 1, H, W) for grayscale
...
def sequential_zero_check(initial_best_params, init_state,
                          ...
                          seed=9999):
    """
    Check each parameter in param_list; set it to 0 if it improves val accuracy.
    """
...
if __name__ == "__main__":
    ...
    # ---------------------------
    # Check if using max num_augmented_copies helps
    # ---------------------------
    ...
    final_save_path = os.path.join(RESULTS_DIR, "FashionMNIST_train_augmented_tensorset.pth")
    ...
    tqdm.write("[main] Grayscale augmentation search and final evaluation complete!")
